...
void LightGroup::TransferConstBuffer()
{
	...
	if (SUCCEEDED(result)) {
		...
		// 丸影
		for (int i = 0; i < ShadowCircleNum; i++) {
			// 有効なら設定を転送
			if (shadowCircles[i].IsActive()) {
				constMap->shadowCircles[i].active = 1;
				constMap->shadowCircles[i].dir = -shadowCircles[i].GetDir();
				constMap->shadowCircles[i].casterPos = shadowCircles[i].GetCasterPos();
				constMap->shadowCircles[i].distanceCasterLight = shadowCircles[i].GetDistanceCasterLight();
				constMap->shadowCircles[i].atten = shadowCircles[i].GetAtten();
				constMap->shadowCircles[i].factorAngleCos = shadowCircles[i].GetFactorAngleCos();
			}
			// ライトが無効ならライト色を0に
			else {
...
{
	assert(0 <= index && index < ShadowCircleNum);

	shadowCircles[index].SetActive(active);
}
...
void LightGroup::SetCircleShadowCasterPos(int index, const XMFLOAT3& casterPos)
{
	...
	shadowCircles[index].SetCasterPos(casterPos);
	dirty = true;
}
...
void LightGroup::SetCircleShadowDir(int index, const XMVECTOR& lightdir)
{
	...
	shadowCircles[index].SetDir(lightdir);
	dirty = true;
}
...
void LightGroup::SetCircleShadowDistanceCasterLight(int index, float distanceCasterLight)
{
	...
	shadowCircles[index].SetDistanceCasterLight(distanceCasterLight);
	dirty = true;
}
...
void LightGroup::SetCircleShadowAtten(int index, const XMFLOAT3& lightAtten)
{
	...
	shadowCircles[index].SetAtten(lightAtten);
	dirty = true;
}
...
void LightGroup::SetCircleShadowFactorAngle(int index, const XMFLOAT2& lightFactorAngle)
{
	...
	shadowCircles[index].SetFactorAngle(lightFactorAngle);
	dirty = true;
}
